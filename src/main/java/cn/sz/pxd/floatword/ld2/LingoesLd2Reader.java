...
public class LingoesLd2Reader {
    ...
    private static final SensitiveStringDecoder[] AVAIL_ENCODINGS = {
            new SensitiveStringDecoder(StandardCharsets.UTF_8),
            new SensitiveStringDecoder(StandardCharsets.UTF_16LE),
            new SensitiveStringDecoder(StandardCharsets.UTF_16BE),
    ...
    public static void main(final String[] args) throws IOException {
        ...
        System.out.println("类型：" + new String(dataRawBytes.array(), 0, 4, StandardCharsets.US_ASCII));
        ...
        if (dataRawBytes.limit() > offsetData) {
            ...
            if (type == 3) {
                ...
                LingoesLd2Reader.readDictionary(dataRawBytes, offsetData);
            } else if (dataRawBytes.limit() > (offsetWithInfo - 0x1C)) {
                LingoesLd2Reader.readDictionary(dataRawBytes, offsetWithInfo);
            } else {
    ...
    private static void decompress(
            final String inflatedFile, final ByteBuffer data,
            final int offset, final int length, final boolean append)
            throws IOException {
        final Inflater inflation = new Inflater();
        try (final InflaterInputStream in = new InflaterInputStream(
                new ByteArrayInputStream(data.array(), offset, length), inflation, 1024 * 8);
    ...
    private static SensitiveStringDecoder[] detectEncodings(
            ...
            final int offsetXml, final int defTotal,
            ...
            for (int k = 0; k < LingoesLd2Reader.AVAIL_ENCODINGS.length; k++) {
                try {
                    for (int i = 0; i < test; i++) {
                        LingoesLd2Reader.readDefinitionData(inflatedBytes, offsetWords, offsetXml, LingoesLd2Reader.AVAIL_ENCODINGS[j],
    ...
    private static void extract(
            final String inflatedFile, final String indexFile,
            final String extractedWordsFile, final String extractedXmlFile,
            final String extractedOutputFile, final int[] idxArray,
            final int offsetDefs, final int offsetXml)
            throws IOException {
        ...
        try (RandomAccessFile file = new RandomAccessFile(inflatedFile, "r");
            ...
            final SensitiveStringDecoder[] encodings =
                    LingoesLd2Reader.detectEncodings(dataRawBytes, offsetDefs,
                            offsetXml, defTotal, idxData, defData);

            dataRawBytes.position(8);

            for (int i = 0; i < defTotal; i++) {
                LingoesLd2Reader.readDefinitionData(dataRawBytes, offsetDefs,
                        offsetXml, encodings[0],
    ...
    private static void getIdxData(
            final ByteBuffer dataRawBytes,
            final int position,
            final int[] wordIdxData) {
    ...
    private static void inflate(
            final ByteBuffer dataRawBytes,
            final List<Integer> deflateStreams,
        ...
        try {
            for (final Integer offsetRelative : deflateStreams) {
                ...
                LingoesLd2Reader.decompress(inflatedFile, dataRawBytes, lastOffset,
                        offset - lastOffset, append);
                append = true;
                lastOffset = offset;
            }
        } catch (final Throwable e) {
            System.err.println("解压缩失败: 0x" + Integer.toHexString(offset) + ": " + e);
        }
    }

    private static void readDefinitionData(
            final ByteBuffer inflatedBytes,
            final int offsetWords,
            final int offsetXml,
            final SensitiveStringDecoder wordStringDecoder,
            final SensitiveStringDecoder xmlStringDecoder,
            final int[] idxData,
            final String[] defData,
            final int i) {
        LingoesLd2Reader.getIdxData(inflatedBytes, 10 * i, idxData);
        ...
        StringBuilder xml = new StringBuilder(LingoesLd2Reader.strip(new String(xmlStringDecoder.decode(inflatedBytes.array(),
                offsetXml + lastXmlPos, currenXmlOffset - lastXmlPos))));
        while (refs-- > 0) {
            final int ref = inflatedBytes.getInt(offsetWords + lastWordPos);
            LingoesLd2Reader.getIdxData(inflatedBytes, 10 * ref, idxData);
            lastXmlPos = idxData[1];
            currenXmlOffset = idxData[5];
            if (xml.length() == 0) {
                xml = new StringBuilder(LingoesLd2Reader.strip(
                        new String(xmlStringDecoder.decode(inflatedBytes.array(),
                        offsetXml + lastXmlPos, currenXmlOffset - lastXmlPos))));
            } else {
                xml.insert(0, LingoesLd2Reader.strip(
                        new String(xmlStringDecoder.decode(inflatedBytes.array(),
                        offsetXml + lastXmlPos, currenXmlOffset - lastXmlPos)))
                        + ", ");
            }
        ...
        defData[1] = xml.toString();

        final String word = new String(wordStringDecoder.decode(inflatedBytes.array(),
                offsetWords + lastWordPos, currentWordOffset - lastWordPos));
        defData[0] = word;
    }
    ...
    private static void readDictionary(
        ...
        System.out.println("XML地址/大小（解压缩后）：0x" + Integer.toHexString(inflatedWordsIndexLength
                + inflatedWordsLength) + " / " + inflatedXmlLength + " B");
        System.out.println("文件大小（解压缩后）：" + ((inflatedWordsIndexLength + inflatedWordsLength
                + inflatedXmlLength) / 1024) + " KB");
        String fileDirPath = "F:\\Document\\My\\English Vocabulary for Postgraduate Examination 2009.ld2";
        final String inflatedFile = fileDirPath + ".inflated";
        LingoesLd2Reader.inflate(dataRawBytes, deflateStreams, inflatedFile);

        if (new File(inflatedFile).isFile()) {
            final String indexFile = fileDirPath + ".idx";
            final String extractedFile = fileDirPath + ".words";
            final String extractedXmlFile = fileDirPath + ".xml";
            final String extractedOutputFile = fileDirPath + ".output";
            ...
            LingoesLd2Reader.extract(inflatedFile, indexFile, extractedFile,
                    extractedXmlFile, extractedOutputFile, idxArray, inflatedWordsIndexLength,
    ...
    private static String strip(final String xml) {
        ...
        if ((open = xml.indexOf("<![CDATA[")) != -1) {
            ...
            if ((end = xml.indexOf("</Ô", open)) != -1) {
                ...
                return xml.substring(open + 1, end)
                        .replace('\t', ' ')
                        .replace('\n', ' ')
                        .replace('\u001e', ' ')
                        .replace('\u001f', ' ');
